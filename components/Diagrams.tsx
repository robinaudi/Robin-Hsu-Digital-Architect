...
export const CertificationGrid: React.FC<{ theme?: string }> = ({ theme = 'dark' }) => {
    ...
    // Styles
    let cardClass = "group p-4 bg-stone-800 border border-stone-700 rounded-lg hover:border-nobel-gold transition-colors";
    let codeClass = "font-bold text-nobel-gold text-lg";
    let iconClass = "text-stone-500 group-hover:text-nobel-gold transition-colors";
    let nameClass = "text-stone-300 text-sm font-medium leading-tight mb-1";
    let orgClass = "text-stone-500 text-xs";

    if (isProfessional) {
        cardClass = "group p-4 bg-white border border-gray-300 rounded-lg hover:border-[#0a66c2] hover:shadow-md transition-all";
        codeClass = "font-bold text-[#0a66c2] text-lg"; // LinkedIn Blue
        iconClass = "text-gray-400 group-hover:text-[#0a66c2] transition-colors";
        nameClass = "text-gray-800 text-sm font-medium leading-tight mb-1";
        orgClass = "text-gray-500 text-xs";
    }
            ...
            {certs.map((c, i) => (
                <div key={i} className={cardClass}>
                    <div className="flex items-center justify-between mb-2">
                        ...
                        <Award size={16} className={iconClass} />
                    </div>
